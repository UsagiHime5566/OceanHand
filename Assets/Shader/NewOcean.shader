Shader "Unlit/NewOcean"
{
    Properties
    {
        _MainTex ("Texture", 2D) = "white" {}
        iChannel0("iChannel0", 2D) = "white" {}
        _Frags ("Frags", Float) = 1.0
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            static float waterlevel = 70.0;        // height of the water
            static float wavegain   = 1.0;       // change to adjust the general water wave level
            static float large_waveheight = 1.0; // change to adjust the "heavy" waves (set to 0.0 to have a very still ocean :)
            static float small_waveheight = 1.0; // change to adjust the small waves

            static float3 fogcolor    = float3( 0.5, 0.7, 1.1 );              
            static float3 skybottom   = float3( 0.6, 0.8, 1.2 );
            static float3 skytop      = float3(0.05, 0.2, 0.5);
            static float3 reflskycolor= float3(0.025, 0.10, 0.20);
            static float3 watercolor  = float3(0.2, 0.25, 0.3);

            static float3 light       = normalize( float3(  0.1, 0.25,  0.9 ) );
            ...
            {
                //return Hash21(p);
                return tex2Dlod (iChannel0, float4(p * (_Frags/256.), 0, 0)).x;
                //return textureLod(iChannel0,p*float2(1./256.),0.0).x;
            }
            ...
            {
                float2 uv = i.uv; uv = 2.0 * uv - 1.0;
                float4 fragColor;
                float2 fragCoord = i.uv * 400;
                float2 iResolution = float2(400, 400);
