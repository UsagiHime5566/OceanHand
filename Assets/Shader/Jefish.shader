    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed2 sph(fixed3 ro, fixed3 rd, fixed3 pos, float radius) {
                ...
                if (det < 0.0) return fixed2(MAX_DISTANCE, MAX_DISTANCE);
            ...
            fixed3 background(fixed3 r) {
                ...
                float x = atan2(r.z, r.x);		// from -pi to pi	
            ...
            de CastRay(ray r) {
                ...
                for(float i=0.; i<MAX_STEPS; i++) {
                    p = r.o + r.d*d;
                        ...
                        if(Dist(q.p.xz, r.d.xz, fixed2(0.,0.))<1.1)
            ...
            fixed4 JellyTex(fixed3 p) { 
                fixed3 s = fixed3(atan2(p.z, p.x), length(p.xz), p.y);
                ...
                b2 = S(.1, .6, b2);
                return fixed4(b+b2,b+b2,b+b2,b+b2);
            }
                ...
                if(o.m>0.) {
                    ...
                    if(o.m==1.) {	// hood color
                        ...
                        for(float i=0.; i<VOLUME_STEPS; i++) {
                            float sd = sph(o.uv, camRay.d, fixed3(0.,0,0), .8+i*.015).x;
                            if(sd!=MAX_DISTANCE) {
                                fixed2 intersect = o.uv.xz+camRay.d.xz*sd;

                                fixed3 uv = fixed3(atan2(intersect.y, intersect.x), length(intersect.xy), o.uv.z);
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                fixed3 camPos = mul(fixed3(0., INVERTMOUSE*camDist*cos((m.y)*pi), camDist),rotX);
                
                CameraSetup(uv, camPos+lookAt, lookAt, 1.);
                
                fixed3 col = render(uv, cam.ray, 0.);
                
                col = pow(col, fixed3(lerp(1.5, 2.6, SIN(t+pi)),lerp(1.5, 2.6, SIN(t+pi)),lerp(1.5, 2.6, SIN(t+pi))));		// post-processing
