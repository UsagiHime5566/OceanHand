Shader "Custom/Ocean"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed3 fogcolor    = fixed3( 0.5, 0.7, 1.1 );              
            fixed3 skybottom   = fixed3( 0.6, 0.8, 1.2 );
            fixed3 skytop      = fixed3(0.05, 0.2, 0.5);
            fixed3 reflskycolor= fixed3(0.025, 0.10, 0.20);
            fixed3 watercolor  = fixed3(0.2, 0.25, 0.3);

            fixed3 light       = normalize( fixed3(  0.1, 0.25,  0.9 ) );
            ...
            {
                //return textureLod(_MainTex,p*fixed2(1./256.),0.0).x;
                return tex2Dlod(_MainTex,float4(p*fixed2(1./256.,1./256.),0.0,0)).x;
            }
            ...
            fixed2x2 m2 = fixed2x2(1.6,-1.2,1.2,1.6);
            ...
            float water( fixed2 p )
            {
                float height = waterlevel;

                fixed2 shift1 = 0.001*fixed2( _Time.y*160.0*2.0, _Time.y*120.0*2.0 );
                fixed2 shift2 = 0.001*fixed2( _Time.y*190.0*2.0, -_Time.y*130.0*2.0 );
            ...
            float trace_fog(in fixed3 rStart, in fixed3 rDirection )
            {
                #if RENDER_CLOUDS
                    // makes the clouds moving...
                    fixed2 shift = fixed2( _Time.y*80.0, _Time.y*60.0 );
                    float sum = 0.0;
                    ...
                    for (int q=0; q<10; q++)
                    {
                        ...
                        fixed3 cpos = rStart + c*rDirection + fixed3(831.0, 321.0+q3-shift.x*0.2, 1330.0+shift.y*3.0); // cloud position
            ...
            bool trace(in fixed3 rStart, in fixed3 rDirection, in float sundot, out float fog, out float dist)
            {
                float h = 20.0;
                float t = 0.0;
                float st = 1.0;
            ...
            fixed3 camera( float time )
            {
                return fixed3( 500.0 * sin(1.5+1.57*time), 0.0, 1200.0*time );
            }
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                fixed2 xy = -1.0 + 2.0*fragCoord.xy / iResolution.xy;
                fixed2 s = xy*fixed2(1.75,1.0);

                // get camera position and view direction
                float time = (_Time.y+13.5+44.)*.05;
                fixed3 campos = camera( time );
                fixed3 camtar = camera( time + 0.4 );
                ...
                fixed3 rd = normalize( s.x*cu + s.y*cv + 1.6*cw );
                ...
                if (!trace(campos,rd,sundot, fog, dist))
                {
                    ...
                    // sun
                    col += 0.47*fixed3(1.6,1.4,1.0)*pow( sundot, 350.0 );
                    // sun haze
                    col += 0.4*fixed3(0.8,0.9,1.0)*pow( sundot, 2.0 );
                        ...
                        for (int q=1000; q<1100; q++) // 100 layers
                        {
                            ...
                            fixed3 cpos = campos + c*rd + fixed3(831.0, 321.0+float(q-1000)*.15-shift.x*0.2, 1330.0+shift.y*3.0); // cloud position
                            float alpha = smoothstep(0.5, 1.0, fbm( cpos*0.0015 ))*.9; // fracal cloud density
                            fixed3 localcolor = lerp(fixed3( 1.1, 1.05, 1.0 ), 0.7*fixed3( 0.4,0.4,0.3 ), alpha); // density color white->gray
                        ...
                        sum.rgb -= 0.6*fixed3(0.8, 0.75, 0.7)*pow(sundot,13.0)*alpha;
                        // This brightens up the low-density parts (edges) of the clouds (simulates light scattering in fog)
                        sum.rgb += 0.2*fixed3(1.3, 1.2, 1.0)* pow(sundot,5.0)*(1.0-alpha);

                        col = lerp( col, sum.rgb , sum.w*(1.0-t) );
                    #endif

                    // add god-rays
                    col += fixed3(0.5, 0.4, 0.3)*fog;
                }
                else
                {
                    #if RENDER_WATER        
                        ...
                        fixed3 wpos = campos + dist*rd; // calculate position where ray meets water

                        // calculate water-mirror
                        fixed2 xdiff = fixed2(0.1, 0.0)*wavegain*4.;
                        fixed2 ydiff = fixed2(0.0, 0.1)*wavegain*4.;

                        // get the reflected ray direction
                        rd = reflect(rd, normalize(fixed3(water(wpos.xz-xdiff) - water(wpos.xz+xdiff), 1.0, water(wpos.xz-ydiff) - water(wpos.xz+ydiff))));  
                        float refl = 1.0-clamp(dot(rd,fixed3(0.0, 1.0, 0.0)),0.0,1.0);
                        ...
                        col += fixed3(.003, .005, .005) * (wpos.y-waterlevel+30.);

                        // Sun
                        float wsunrefl = wsky*(0.5*pow( sundot, 10.0 )+0.25*pow( sundot, 3.5)+.75*pow( sundot, 300.0));
                        col += fixed3(1.5,1.3,1.0)*wsunrefl; // sun reflection

                    #endif

                    // global depth-fog
                    float fo = 1.0-exp(-pow(0.0003*dist, 1.5));
                    fixed3 fco = fogcolor + 0.6*fixed3(0.6,0.5,0.4)*pow( sundot, 4.0 );
                    col = lerp( col, fco, fo );
